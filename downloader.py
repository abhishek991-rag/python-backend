...
def download_video(url, format_choice="bestvideo+bestaudio/best"):
    """
    ...
    ydl_opts = {
        ...
        # %(id)s - video ID (useful for unique filenames if titles clash)
        'outtmpl': os.path.join(DOWNLOAD_DIR, '%(title)s (%(id)s).%(ext)s'),
        'noplaylist': True, # Ensure only single video downloads, not playlists by default
        'progress_hooks': [lambda d: print(f"YT-DLP Progress: {d['status']} - {d.get('_percent_str', '')} - {d.get('filename', '')}")], # Detailed progress
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # --- DEBUGGING OUTPUT ---
            # This is crucial for understanding what yt-dlp processed and downloaded
            ...
            # ydl.prepare_filename() is the most reliable way to get the actual path after all post-processing
        ...
        # Specific error for yt-dlp download failures (e.g., video unavailable, geo-restricted)
...
if __name__ == "__main__":
    ...
    # youtube_video_url = "https://www.youtube.com/watch?v=some_invalid_id" # Test invalid YouTube URL
    ...
    print("\nAttempting Instagram reel download...")
    # NOTE: Replace with a public Instagram reel URL. Private reels will likely fail.
    # If it consistently fails, it might be due to geo-restrictions or private content.
    instagram_url = "https://www.instagram.com/reel/C71_B_TIZWk/" # Example, find a public one
    result_instagram = download_video(instagram_url, format_choice="bestvideo+bestaudio/best")
    ...
    print("\nAttempting Facebook video download...")
    ...
    # A real example might be from a news page or public celebrity page
    # facebook_url = "https://www.facebook.com/cnn/videos/123456789/" # Example, please verify a real public video
    result_facebook = download_video(facebook_url, format_choice="bestvideo+bestaudio/best")
